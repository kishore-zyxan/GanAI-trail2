...
async def upload_files(files: List[UploadFile] = File(...)):
    responses = []
    request_id = uuid4().hex  # One ID per request
    ...
    for file in files:
        ...
        try:
            ...
            file_id = uuid4().hex  # Unique per file

            # Store in DB
            insert_into_mysql(file.filename, flat_data, request_id, file_id, upload_time)
    ...
    return JSONResponse(content={"request_id": request_id, "results": responses})
